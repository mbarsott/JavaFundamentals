...
public class JF03_VarsTypesOperators {
  ...
  public static void main(String[] args) {
    ...
    // Java is a "strongly typed" language, so every variable has a type and it cannot be changed
    ...
    System.out.print("The value of i is "); // this type of literal is called a string literal.
    ...
    // You can use variables to create mathematical expressions, just like in algebra
    System.out.println((i + j) * thisIsAnotherIntegerVariable); // prints 100 (* is multiplication)
    // You can assign expressions to variables:
    int k = (i + j) * thisIsAnotherIntegerVariable;
    System.out.println(k); // also prints 100

    // Primitive data types: fundamental types in Java. There are  four categories:
    ...
    // Floating Point Types (allow fractional parts in a number)
    ...
    // When you declare primitive type variables, space is allocated for the value and the variable holds
    // the value stored in that space directly: primitive data types are stored by value! See the example
    int myFirstInt = 1; // space for one int is allocated and 1 is stored there
    int mySecondInt = 2; // space for another int is allocated and 2 is stored there
    ...
    now = oneHourAgo; // after the assignment now and oneHourAgo both contain a reference the same Date object
    System.out.println("oneHourAgo was assigned to now");
    System.out.println(now); // prints date and time of one hour ago
    System.out.println(oneHourAgo); // prints date and time of one hour ago
    now.setTime(now.getTime() + 60 * 60 * 1000); // sets the time of the now object, adding one hour.
    ...
    System.out.println(now); // prints current date and time (one hour after one hour ago)
    System.out.println(oneHourAgo); // prints current date and time (same as now, since it references the same object)
    ...
    // Prefix increment (++variable) increments the variable and returns the result
    i = 10;
    double x = 3.141516;
    System.out.println(++i); // Increments the value of i to 11 and prints 11
    System.out.println(++x); // prints 4.141516
    // Postfix decrement (variable--) returns the original value of the variable and then decrements it
    ...
    System.out.println(++i + 2 * 5 % i++); // (++i) + ((2 * 5) % (i++)) => (2 + (10 % 2)) => 2 + 0 => prints 2
    System.out.println(i); // prints 3: i value was 1, then it was prefix incremented and postfix incremented above.
    i = 1; // i was 3 at the previous statement. Lets reset it to 1.
    System.out.println((++i) + ((2 * 5) % (i++))); // Just checking if parenthesis also work the same => prints 2
  }
}
